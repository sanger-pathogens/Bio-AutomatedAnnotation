...
sub annotate
{
  ...
  if ($kingdom ne 'Viruses') {
    ...
    for my $entry (@{$data->{entries}[0]->{entry}}) {
      ...
      $self->msg(join "\t", $num_rrna, $desc, $sid, $entry->{start}[0], $entry->{stop}[0], $entry->{direction}[0]);
    }
  ...
  $self->runcmd("tbl2asn -N 1 -y 'Annotated using $EXE $VERSION from $URL' -Z $outdir/$prefix.err -M n -V b -i $outdir/$prefix.fsa -f $outdir/$prefix.tbl 2> /dev/null");
