package Bio::AutomatedAnnotation::ParseInterProOutput;
...
parse the GFF files produced by interproscan
   ...
   my $obj = Bio::AutomatedAnnotation::ParseInterProOutput->new(
...
{
  my ($self) = @_;
  open(my $fh, '>', $self->output_file) or Bio::AutomatedAnnotation::Exceptions::CouldntWriteToFile->throw(
...
sub _input_single_gff_file_fh
{
  ...
  open(my $fh,'-|', 'cat '.$filename. ' | '.$self->_remove_sequence_filter ) or Bio::AutomatedAnnotation::Exceptions::FileNotFound->throw(
